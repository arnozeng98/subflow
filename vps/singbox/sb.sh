...
SINGBOX_RELEASE_REPO="arnozeng98/subflow"
...
# >>>>>>>>> BEGIN MODULE: 00_dependencies.sh <<<<<<<<<<<
...
# >>>>>>>>> END MODULE: 00_dependencies.sh <<<<<<<<<<<

# >>>>>>>>> BEGIN MODULE: 00_generated.sh <<<<<<<<<<<
...
get_release_latest_tag() {
  local repo="${SINGBOX_RELEASE_REPO:-arnozeng98/subflow}"
...
install_or_update_singbox() {
  ...
  base_url="https://github.com/${SINGBOX_RELEASE_REPO:-arnozeng98/subflow}/releases/download/${tag}"
...
main_menu() {
  ...
  while true; do
    clear
    print_rect_title "Sing-box Elite 管理系统"
    singbox_status_summary
    echo -e "${B}--------------------------------------------------------${NC}"
