...
PACKAGE_ROOT="${INSTALL_ROOT}/subflow"   # python package (vps/subflow)
CLI_ROOT="${INSTALL_ROOT}/cli"           # menu.sh + lib.sh + uninstall.sh
PAGES_ROOT="${INSTALL_ROOT}/pages"       # Cloudflare Pages assets (cloudflare/functions)
ENV_DIR="/etc/subflow"
ENV_FILE="${ENV_DIR}/subflow.env"
SYSTEMD_UNIT="/etc/systemd/system/subflow.service"
SF_LINK="/usr/local/bin/sf"

# Bundled sing-box manager (vendored from vps/singbox). subflow ships and
# installs it directly, so the box no longer depends on any external installer.
...
# Sets SOURCE_ROOT to a directory containing vps/subflow + vps/deploy.
prepare_source_tree() {
...
copy_runtime() {
  ...
  # Bundle the vendored sing-box manager so the box is self-contained.
...
# Bootstrap the bundled sing-box manager: place sb.sh at its canonical path and
# expose the `s` shortcut. Idempotent; safe to re-run on update.
...
# True when a working sing-box (built with v2ray_api) is already installed.
singbox_installed() {
