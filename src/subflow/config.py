...
def load_config() -> AppConfig:
  ...
  Path defaults mirror the upstream Tangfffyx/sing-box layout so a fresh subflow
  instance attaches to an existing VPS without re-discovering file locations.
  ...
  return AppConfig(
    listen_host=os.environ.get("SUBFLOW_LISTEN_HOST", "127.0.0.1").strip() or "127.0.0.1",
    listen_port=_read_int("SUBFLOW_LISTEN_PORT", 28080),
